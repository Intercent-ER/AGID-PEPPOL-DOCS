...
1. *cac:TaxCategory/cbc:ID* (BT-118 VAT category code);
2. *cbc:TaxExemptionReasonCode* (BT-121 VAT exemption reason code);
3. *cbc:TaxExemptionReason* (BT-120 VAT exemption reason text), in cui viene inserita il codice natura secondo le specifiche FatturaPA ed il Riferimento Normativo _concatenati a mezzo di #._
