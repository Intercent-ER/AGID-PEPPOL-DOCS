...
* Inserito nuovo profilo opzionale Profilo 02 – Fatturazione con risposta che consente di gestire un processo in cui a seguito della ricezione della fattura, è richiesto l'invio di una risposta alla fattura. Questo profilo richiede una registrazione separata nell'SMP e utilizza il documento di Invoice Response BIS. Le specifiche di questo documento sono disponibili sul portale specifiche di https://docs.peppol.eu/poacc/other/[OpenPeppol Other BIS].
...
* Modifica da «warning» a «fatal» la severità delle regole DK-R-017 (Danimarca) e DK-R-003 (Danimarca).
...
* Aggiunti nella regola PEPPOL-EN16931-R007 (Francia) 2 nuovi profili temporanei per Fattura UBL e Nota di Credito
