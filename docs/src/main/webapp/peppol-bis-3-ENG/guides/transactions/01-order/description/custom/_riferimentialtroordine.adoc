...
In the case of *Order Connection*, *Order Revocation* or *Order Replacement*, it is necessary to report the so-called "Identification triple"(in italian Tripletta di identificazione) of the related Order, that for example needs to be revoked or replaced, in the elemement `cac:OrderDocumentReference/cbc:ID`, which is a structured field, by concatenating the values and by respecting the order of priority that follows:
