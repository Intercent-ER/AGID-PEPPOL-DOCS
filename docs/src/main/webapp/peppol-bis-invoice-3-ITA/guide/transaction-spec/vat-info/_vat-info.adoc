...
Ogni riga della fattura deve avere il codice della categoria IVA riferita alla voce fatturata (BT-151 VAT category code cac:ClassifiedTaxCategory/cbc:ID). +
Se la categoria IVA è Z o E o G o K o AE, il BT-152 Invoiced item VAT rate (cac:ClassifiedTaxCategory/cbc:Percent) va posto uguale a 0.

=== Regole per la fatturazione domestica

#Ogni riga della fattura deve avere il codice della categoria IVA riferita alla voce fatturata (BT-151 VAT category code cac:ClassifiedTaxCategory/cbc:ID).#+
#Se la categoria IVA è S o B, l’aliquota IVA va inserita in BT-152 Invoiced item VAT rate (cac:ClassifiedTaxCategory/cbc:Percent).#

#Per le fatture domestiche (BT-40 Seller country code pari a IT e BT-55 Buyer country code pari a IT) la gestione della Natura della riga è operata a mezzo dell’utilizzo del *cac:AdditionalItemProperty/cbc:Name* (BT-160) e *cac:AdditionalItemProperty/cbc:Value* (BT-161) per indicare la natura a livello di riga di fattura .# +
#L’item attribute name viene valorizzato con la stringa “NATURA”, l’item attribute value con il codice natura secondo le specifiche FatturaPA.#
...
La gestione della natura dell’operazione (FPA 2.2.2.2 Natura) per le fatture cross-border (BT-40 Seller country code diverso da IT e BT-55 Buyer country code pari a IT)
è operata a mezzo dell’utilizzo del *cac:TaxCategory/cbc:ID* (BT-118 VAT category code) per i riepiloghi IVA e *cac:Subtotal/cbc:TaxExemptionReasonCode* (BT-121 VAT VAT exemption reason code).
