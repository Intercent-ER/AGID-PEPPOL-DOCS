...
[lime-background]#Per evidenziare che il totale fattura non include l'IVA (a carico del committente), si deve utilizzare il *cbc:PrepaidAmount* indicando il valore delle tasse. Si evidenzia che il cbc:PrepaidAmount non è supportato in FatturaPA ma viene utilizzato solo per validare il calcolo del totale fattura.#
