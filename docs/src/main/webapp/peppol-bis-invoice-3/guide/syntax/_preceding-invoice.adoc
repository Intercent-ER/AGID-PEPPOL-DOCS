...
----
<1> L'identificatore è obbligatorio se viene incluso il `cac:B_illingReference`
<2> La data di emissione deve essere compilata se il riferimento alla fattura non è univoco
<3> Ripete il `cac:B_illingReference` per aggiungere diversi riferimenti a fatture precedenti
