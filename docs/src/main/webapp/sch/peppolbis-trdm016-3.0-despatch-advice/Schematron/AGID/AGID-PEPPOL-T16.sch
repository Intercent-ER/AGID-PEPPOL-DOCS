...
<pattern id="IT-UBL-T16" xmlns="http://purl.oclc.org/dsdl/schematron">
	...
	<rule context="//cbc:EndpointID" flag="fatal">
		...
		<assert test="(@schemeID='9906' and matches(.,'^(IT)?[0-9]{11}$')) or @schemeID!='9906'" flag="fatal" id="IT-T16-R027">[IT-T16-R027] - Se l'identificatore di endpoint si basa sullo schema di identificatori IT:VAT (ICD: 9906), questo dovrà seguire la sintassi (IT)?[0-9]{11}.</assert>
		<assert test="(@schemeID='9907' and matches(.,'^[0-9]{11}$|^[A-Z]{6}\d{2}[ABCDEHLMPRST]{1}\d{2}[A-Z]{1}\d{3}[A-Z]{1}$')) or @schemeID!='9907'" flag="fatal" id="IT-T16-R028">[IT-T16-R028] - Se l'identificatore di endpoint si basa sullo schema di identificatori IT:CF (ICD: 9907), questo dovrà seguire la sintassi [0-9]{11} per le persone giuridiche e la sintassi [A-Z]{6}\d{2}[ABCDEHLMPRST]{1}\d{2}[A-Z]{1}\d{3}[A-Z]{1} per quelle fisiche.</assert>
	</rule>
	...
	<rule context="/ubl:DespatchAdvice" flag="fatal"> 
		<assert test="cbc:CustomizationID = 'urn:fdc:peppol.eu:poacc:trns:despatch_advice:3:extended:urn:www.agid.gov.it:trns:ddt:3.0' and cbc:ProfileID = 'urn:fdc:peppol.eu:poacc:bis:despatch_advice:3'" flag="fatal" id="IT-T16-R004">[IT-T16-R004] - La versione del DespatchAdvice (DDT) DEVE essere identificata dai seguenti identificativi: 
