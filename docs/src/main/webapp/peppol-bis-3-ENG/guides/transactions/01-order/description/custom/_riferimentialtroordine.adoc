...
In the case of a *Connected Order*, *Cancelled Order* or *Revised Order*, it is necessary to report the so-called "Identification triple"(in italian Tripletta di identificazione) of the related document or the order that needs to be revoked or replaced, in the elemement `cac:OrderDocumentReference/cbc:ID`, which is a structured field, by concatenating the values and by respecting the order of priority that follows:
